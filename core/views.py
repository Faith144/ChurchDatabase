                ...
                'units': [
                    {
                        ...
                        'name': unit.name,
                        'type': 'Unit',
                        'description': unit.description[:100] + '...' if unit.description and len(unit.description) > 100 else unit.description,
                    }
                    for unit in units
                ],
                'cells': [
                    {
                        ...
                        'name': cell.name,
                        'type': 'Cell',
                        'created_at': cell.created_at.strftime('%Y-%m-%d') if cell.created_at else '',
                    }
                    for cell in cells
                ],
            }
...
def get_member_form(request, pk=None):
    ...
    try:
        ...
        html = render_to_string('dashboard/partials/member_form_modal.html', {
            'form': form,
            'title': title,
            'member_id': pk
        })
...
def get_cell_form(request, pk=None):
    ...
    try:
        ...
        html = render_to_string('dashboard/partials/cell_form_modal.html', {
            'form': form,
            'title': title,
            'cell_id': pk
        })
...
def delete_cell(request, pk):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            return JsonResponse({
                'success': True,
                'message': f'Cell {cell_name} deleted successfully!'
            })
        else:
            return JsonResponse({
                'success': False,
...
def get_assembly_form(request, pk=None):
    ...
    try:
        ...
        html = render_to_string('dashboard/partials/assembly_form_modal.html', {
            'form': form,
            'title': title,
            'assembly_id': pk
        })
...
def public_registration(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Thank you for registering! Your information has been submitted for review.')
...
def unit_list(request):
    """List all units"""
    units = Unit.objects.annotate(member_count=Count('members')).select_related('leader')
    context = {'units': units}
...
def get_unit_form(request, pk=None):
    ...
    try:
        ...
        html = render_to_string('dashboard/partials/unit_form_modal.html', {
            'form': form,
            'title': title,
            'unit_id': pk
        })
